...
- Add NTFS/MFT acceleration only as an optional experimental path. `clean --scan-backend windows-ntfs-mft-experimental` attempts a read-only live NTFS volume index on supported local NTFS volumes, tries sequential `$MFT` reads before per-record FSCTL reads, expands valid stream-backed `$INDEX_ALLOCATION:$I30` directory indexes through Rebecca-owned parser/index DTOs, reuses the index within a command, and falls back with caveats when unsupported, unprivileged, or ambiguous.
