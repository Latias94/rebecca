...
- Branch: feat/cli-surface-completion
- Last verified: Scan engine deep-module refactor passed on 2026-06-29 with `cargo check -p rebecca-core --benches`, targeted scan/planner/cache/CLI nextest, `cargo nextest run --workspace` (362 tests), `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `git diff --check`, and engineering memory validation.
...
- Cleanup-core maturity baseline: Planner scans and cleanup execution now use bounded shared parallelism; scan-cache records are treated as rebuildable optimization data with explicit miss, expiry, corruption, pruning, and human-summary reporting; execution revalidation classifies protected targets as `safety-policy-blocked`, disappeared targets as `execution-target-missing`, and backend permission/IO errors as `execution-failed`, with history and CLI replay coverage.
- Latest completed: Scan engine deep-module refactor on `feat/cli-surface-completion`: `ScanEngine` is now the caller-facing cleanup measurement module, `ignore::WalkBuilder` is hidden behind a private cleanup traversal adapter, and cleanup measurement counts hidden files plus `.ignore` / `.gitignore` matched files instead of inheriting search-tool filtering.
- In progress: None.
- Blocked: None.
- Next action: Continue with the next Rebecca CLI/GUI integration surface, or benchmark-driven scan adapter work if large-directory performance becomes a measured bottleneck.
