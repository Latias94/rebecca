...
- Last verified: cleanup-core scan-cache pruning follow-up (`cargo nextest run -p rebecca-core`, `cargo nextest run -p rebecca-cli --test cli_clean -p rebecca-cli --test cli_output`, and `cargo nextest run --workspace`) passed on 2026-06-28.
...
- In progress: None.
- Blocked: None.
- Next action: Pick the next cleanup family or `purge` UX follow-up if a real gap appears.
