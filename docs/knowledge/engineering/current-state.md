...
timestamp: 2026-06-24T23:56:00+08:00
status: "active"
---

# Current State

- Goal: Windows install-time verification is now layered on top of the release integrity baseline. Rebecca has CI, release ZIP packaging, SHA-256 checksums, GitHub build-provenance attestations, and a PowerShell installer that verifies checksums before extraction and can require attestation verification.
- Branch: feat/windows-cleanup-mvp
- Last verified: `pwsh -NoProfile -File scripts/release/build-release.ps1 -Tag v0.1.0 -Repository local/rebecca -OutDir target\release-smoke -SkipBuild`, `pwsh -NoProfile -File scripts/release/write-checksums.ps1 -DistDir target\release-smoke`, `pwsh -NoProfile -File scripts/install.ps1 -AssetPath target\release-smoke\rebecca-0.1.0-windows-x86_64-msvc.zip -ChecksumsPath target\release-smoke\SHA256SUMS -InstallDir target\install-smoke -SkipAttestation`, bad-checksum rejection before install-dir creation, required-attestation rejection before install-dir creation, `cargo fmt --all -- --check`, `cargo nextest run --workspace` (242 passing tests), `cargo clippy --workspace --all-targets -- -D warnings`, and `git diff --check` passed on 2026-06-24 for the install verification baseline.
...
- Latest completed: Added `docs/plans/2026-06-24-020-feat-windows-install-verification-plan.md` and `scripts/install.ps1`; updated release packaging to include the install script plus `repository=` metadata; documented latest/tagged install, update-by-rerun, checksum-only install, and `-RequireAttestation` mode in README, `docs/release.md`, `SECURITY.md`, and the safety audit.
- In progress: Distribution hardening beyond the PowerShell installer remains open: MSI/MSIX, PATH automation, package-manager manifests, SBOM generation, Windows ARM64 artifacts, and fully pinned GitHub Action SHAs.
- Blocked: None.
- Next action: After committing the install baseline, choose the next distribution slice: action SHA pinning plus dependency update automation, package-manager publishing, or native installer packaging.
