...
- Last verified: `cargo nextest run -p rebecca-rules`, `cargo nextest run -p rebecca-core --test model_contract`, `cargo nextest run --workspace`, and `cargo clippy --workspace --all-targets -- -D warnings` all passed on 2026-06-24 after tightening rule catalog metadata and built-in rule governance.
...
- Latest completed: Rule catalog validation now rejects empty categories, names, and target paths, while the built-in catalog must stay Windows-scoped, use `windows.` ids, include non-empty restore hints, and use project-owned provenance.
- In progress: Future settings extensions and any next cleanup-rule additions should build on the tightened catalog validator rather than relying on rule-authoring convention alone.
- Blocked: None.
