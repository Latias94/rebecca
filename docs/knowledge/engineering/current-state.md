...
- Last verified: `cargo fmt --all`, `cargo nextest run -p rebecca-windows`, `cargo nextest run -p rebecca-core --test discovery`, `cargo nextest run -p rebecca-cli --test info`, and `cargo nextest run --workspace` passed on 2026-06-24 while hardening Steam discovery command parsing and HKCU `SteamExe` fallback. The latest full-suite verification passed with 121 tests.
