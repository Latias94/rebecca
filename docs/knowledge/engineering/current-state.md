...
- In progress: Steam discovery and cleanup hardening continue after the Steam cleanup expansion slice; the latest follow-up added the Steam install depot cache and logs rules, then simplified Steam install/library target resolution so the shared discovery path stays smaller and easier to extend.
- Blocked: None.
