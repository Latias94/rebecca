...
- Last verified: `cargo fmt --all`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo nextest run --workspace` passed on 2026-06-24 after adding the Steam install depot cache and logs rules, simplifying Steam target resolution, locking Steam restore-hint output, core serialization contracts, explicit category validation, moving selection validation onto `RuleSelection`, tightening the builtin rule catalog loader, simplifying the Steam registry source table, and tightening Steam libraryfolders candidate reading. The latest full-suite verification passed with 142 tests.
...
- In progress: Steam discovery and cleanup hardening continue after the Steam cleanup expansion slice; the latest follow-up added the Steam install depot cache and logs rules, simplified Steam install/library target resolution, tightened the builtin rule catalog loader, simplified the Steam registry source table, tightened Steam libraryfolders candidate reading, removed the redundant Windows Steam installation wrapper, further folded the shared Steam relative-target resolution helper inside core discovery, and aligned the Steam rule coverage baseline to include `windows.steam-install-logs`.
- Blocked: None.
