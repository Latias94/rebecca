...
- In progress: Steam discovery and cleanup hardening continue after the Steam cleanup expansion slice; the latest follow-up added the Steam install depot cache and logs rules, simplified Steam install/library target resolution, tightened the builtin rule catalog loader, simplified the Steam registry source table, and tightened Steam libraryfolders candidate reading so the shared surfaces stay smaller and easier to extend.
- Blocked: None.
