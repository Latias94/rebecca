...
- Last verified: domestic desktop-app cache batch (`cargo fmt --all -- --check`, `cargo nextest run --workspace` with 322 passing tests, and `git diff --check`) passed on 2026-06-27.
...
- In progress: Cleanup catalog expansion remains open: additional conservative Windows cache families, more `purge` discoverability if needed, and any safety-audit tightening that follows from new rule batches.
- Blocked: None.
