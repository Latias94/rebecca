...
timestamp: 2026-06-24T03:17:00Z
status: "active"
---
...
- Last verified: `cargo fmt --all --check`, `cargo nextest run --workspace`, and `cargo clippy --workspace --all-targets -- -D warnings` passed on 2026-06-24 after versioning the config schema contract.
...
- In progress: Configuration and local-state contract work now leads the follow-up work; the latest slice added config schema version `1`, defaulted omitted versions to `1`, rejected unsupported versions through core and CLI regressions, documented a copyable v1 TOML example, and accepted the configuration/local-state ADR.
- Blocked: None.
- Next action: Continue with cache/state lifecycle policy before widening cleanup scope: decide which files are durable state versus disposable cache, then pin the CLI and docs around that boundary.
