...
- Last verified: CLI API contract slice passed on 2026-06-28 with `cargo check -p rebecca-cli`, targeted CLI API/help/completion/output nextest, `cargo nextest run --workspace` (359 tests), `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `git diff --check`.
...
- Latest completed: CLI API v1 contract slice on `feat/cli-surface-completion`: shared global `--format human|json|ndjson`, v1 success/error envelopes, NDJSON cleanup lifecycle/progress/error events, `doctor permissions` diagnostic payload, and Draft 2020-12 schema/docs/examples are implemented and locally gated.
- In progress: Next Rebecca CLI/GUI integration surface selection.
- Blocked: None.
- Next action: Continue with the next Rebecca CLI/GUI integration surface on top of the committed API contract.
