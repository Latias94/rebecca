...
- Last verified: `cargo fmt --all`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo nextest run --workspace` passed on 2026-06-24 after adding the Steam install avatar cache rule, simplifying Steam target resolution, locking Steam restore-hint output, core serialization contracts, explicit category validation, moving selection validation onto `RuleSelection`, tightening the builtin rule catalog loader, simplifying the Steam registry source table, tightening Steam libraryfolders candidate reading, collapsing the Steam planner and CLI regression coverage into table-driven tests, extracting shared Steam test data into repo-level helpers, and adding the conservative Steam install stats, appinfo, localization, and package info cache rules.
...
- In progress: Steam discovery and cleanup hardening continue after the Steam cleanup expansion slice; the latest follow-up added the Steam install avatar cache rule, collapsed the Steam planner and CLI regression coverage into table-driven tests, refreshed the Steam user-facing docs to keep the catalog boundaries explicit, deepened the Steam test structure by moving shared rule cases into a common helper, expanded the catalog with `windows.steam-install-stats-cache`, `windows.steam-install-appinfo-cache`, `windows.steam-install-localization-cache`, and `windows.steam-install-packageinfo-cache`, and kept the shared Steam fixture helper and its callers on borrowed cases instead of copied values.
- Blocked: None.
- Next action: Continue with the next approved follow-up slice after Steam cleanup expansion, now focusing on remaining discovery and cleanup edge cases, contract tightening, or adjacent documentation alignment; if no equally conservative Steam candidate appears, keep tightening shared helpers and docs rather than widening the rule set.
