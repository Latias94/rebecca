...
- In progress: Steam discovery and cleanup hardening continue after the Steam cleanup expansion slice; the latest follow-up added the Steam install avatar cache rule, collapsed the Steam planner and CLI regression coverage into table-driven tests, refreshed the Steam user-facing docs to keep the catalog boundaries explicit, deepened the Steam test structure by moving shared rule cases into a common helper, and expanded the catalog with `windows.steam-install-stats-cache` and `windows.steam-install-appinfo-cache`, with a small follow-up simplification that switched the shared Steam fixture helper and its callers to borrowed cases instead of copied values.
- Blocked: None.
