...
- Last verified: `cargo nextest run -p rebecca-core config`, `cargo nextest run -p rebecca-cli --test cli_output`, and `cargo nextest run -p rebecca-cli --test cli_cache` passed on 2026-06-24 after centralizing the configuration and local-state contract.
...
- Latest completed: The configuration and local-state contract now has a dedicated `docs/configuration.md` reference, with README, ADR, and rule-authoring guidance aligned to the same schema, precedence, lifecycle, migration, and ownership boundaries.
- In progress: Rule catalog hardening and future settings extensions remain the next follow-up areas after closing the configuration/state documentation slice.
- Blocked: None.
