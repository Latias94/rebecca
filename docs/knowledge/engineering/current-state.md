...
- Last verified: `cargo fmt --all`, `cargo nextest run -p rebecca-core --test discovery`, `cargo nextest run -p rebecca-core --test planner`, `cargo nextest run -p rebecca-cli --test info`, and `cargo nextest run --workspace` passed on 2026-06-24 while hardening Steam discovery path handling. The latest full-suite verification now passed with 114 tests.
