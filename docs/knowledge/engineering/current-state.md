...
- In progress: Steam discovery and cleanup hardening continue after the Steam cleanup expansion slice; the latest follow-up added the Steam install depot cache and logs rules, simplified Steam install/library target resolution, and tightened the builtin rule catalog loader so the shared surfaces stay smaller and easier to extend.
- Blocked: None.
