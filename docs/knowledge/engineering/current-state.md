...
- In progress: Steam discovery and cleanup hardening continue after the Steam cleanup expansion slice; the latest follow-up added the Steam install logs rule and kept selection validation centralized on `RuleSelection` so `scan` and `clean` share one semantic entry point.
- Blocked: None.
