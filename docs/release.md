...
- `release-preflight.yml` is a manual gate that validates a chosen source ref and version, checks the crates.io publish order, builds crate package file lists, dry-runs unpublished registry-independent crate publishes, verifies dependent crates with package-context plus exact internal dependency proof, exercises the repository PowerShell release archive scripts, and runs macOS cleanup smoke;
...
The release workflow dry-runs each unpublished crate immediately before publishing it, skips crate versions already visible on crates.io, and waits for each dependency crate to become visible before publishing the next dependent crate. Preflight checks the publish order against normal, build, and dev workspace dependencies because Cargo verifies dev-dependencies while preparing a crate for publication. Preflight can fully `cargo publish --dry-run` only registry-independent crates before anything is published; internally dependent crates instead get a package-context check plus exact `=version` internal dependency proof in `package-verification.json`, then receive the real publish dry-run after their dependencies are visible. GitHub Release hosting waits for crates.io publishing to complete successfully, so a tag has one release status instead of two independent tag-triggered publishers.
