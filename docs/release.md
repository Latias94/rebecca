...
Rebecca uses cargo-dist for tag-driven GitHub Releases and a separate workflow for crates.io publishing. The current downloadable target is Windows x86_64 MSVC.
...
- `release.yml` is generated by cargo-dist and publishes the tag-driven ZIP, PowerShell installer, and checksum files to GitHub Releases;
- `release-crates.yml` publishes `rebecca-core`, `rebecca-rules`, `rebecca-windows`, and `rebecca-cli` to crates.io in dependency order.

## Artifact Names

For tag `v0.1.0`, cargo-dist currently publishes:

```text
rebecca-cli-x86_64-pc-windows-msvc.zip
rebecca-cli-x86_64-pc-windows-msvc.zip.sha256
rebecca-cli-installer.ps1
sha256.sum
source.tar.gz
...
Set `REBECCA_CLI_INSTALL_DIR` to override the install directory. Run the installer for a newer tag to update.
...
The crates.io workflow is intentionally separate from GitHub Release publishing. It dry-runs unpublished crates before publishing, skips crate versions already visible on crates.io, and waits for each dependency crate to become visible before publishing the next dependent crate.
...
```powershell
$asset = "rebecca-cli-x86_64-pc-windows-msvc.zip"
