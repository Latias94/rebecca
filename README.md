...
Use `--format ndjson` for long-running cleanup workflows that need progress events. NDJSON defaults to target-level progress; use `--progress-detail file` for verbose per-file scan events. Use `inspect map --table csv|tsv` when a caller needs flat disk-map rows for Excel, PowerShell, DuckDB, or other table-first tools; add repeated `--table-row total|root|entry|group` when the caller only wants selected row kinds.
