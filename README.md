...
- `clean --scan-backend windows-native` opts into the Windows native directory enumeration backend for plan estimates; `windows-ntfs-mft-experimental` attempts a read-only live NTFS/MFT index on supported local NTFS volumes, tries a sequential `$MFT` source before the per-record FSCTL source, expands valid stream-backed `$INDEX_ALLOCATION:$I30` directory indexes through Rebecca's sequence-aware parser/index model, and falls back to a safe directory scanner with provenance when unsupported, unprivileged, or too ambiguous to trust. `inspect space` accepts the same backend selector for read-only estimates. The default remains the portable cleanup walker.
