...
fn context_project_artifact_definition_for_dir(
    dir: &Path,
    name: &str,
...
fn has_regular_file(path: PathBuf) -> bool {
    let Ok(metadata) = fs::symlink_metadata(path) else {
        return false;
    };
...
fn scan_root(
    ...
    while let Some((dir, depth)) = pending.pop_front() {
        ...
        if let Some(name) = dir.file_name().and_then(|name| name.to_str()) {
            if let Some(definition) = context_project_artifact_definition_for_dir(&dir, name) {
