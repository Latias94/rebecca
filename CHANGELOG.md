...
- `rebecca inspect map --cleanup-advice` now annotates ranked entries with read-only cleanup advice from the cleanup rule catalog and protection policy, including cleanability status, matched rule/protection facts, required opt-ins, and a dry-run command hint.
