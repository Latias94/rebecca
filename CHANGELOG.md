...
- Cleanup execution now mutates targets through evidence-aware state transitions so completed, skipped, blocked, and failed outcomes keep target fields, ordered evidence facts, and summary matrices synchronized.
...
- TUI disk analytics now includes a keyboard-reachable Treemap view (`4`/`w`), treemap drilldown with `Enter`/`l`, explicit non-drillable tile feedback, mouse tab/row/tile selection, mouse-wheel selection movement, and deterministic Treemap snapshot and hit-test coverage while keeping cleanup execution behind preview and typed confirmation.
- TUI disk analytics now includes keyboard-reachable type and extension distribution views, path-stable scoped directory/root refresh, retryable task errors, and headless replay coverage for the new journeys.
...
- TUI background work now runs through a shared task runtime with child cancellation tokens, live scan/planning progress snapshots, target-boundary cleanup cancellation, and structured worker result handling so future interactive views can share the same async/concurrency foundation.
- TUI human sessions now persist private display preferences for last workbench view, sort, entry limit, scan backend, screen-reader mode, and color mode without storing scanned paths; hidden `--once` and replay smoke runs do not write preferences, and `--visual-bars` / `--color` can override saved accessible display choices.
