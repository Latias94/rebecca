...
- `rebecca inspect map --cleanup-advice` now annotates ranked entries with read-only cleanup advice from the cleanup rule catalog, project artifact policy, and protection policy, including cleanability status, matched rule/protection/artifact facts, required opt-ins, and a dry-run command hint.
...
- `scripts/ntfs/run-live-mft-dogfood.ps1` now emits top-level backend comparison summaries with portable-baseline match status, metric deltas, duration ratios, fastest requested/actual backend identity, and bounded diagnostic summary totals.
- `scripts/dogfood/run-inspect-map-report.ps1` now collects local `inspect map` backend evidence under `target/inspect-map-dogfood/`, derives JSON/CSV/Markdown reports from one JSON scan per backend run, and includes a `-SelfTest` parser/report check.
