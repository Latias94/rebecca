...
- Experimental NTFS/MFT volume-index caching now writes versioned `MftIndex` payload files beside configured manifests, with byte length, checksum, schema, generation, and fingerprint validation; persistent payload reads remain gated on future USN freshness validation.
